...
public class PleaseProvideControllerClassName implements MouseListener {
	...
	private void runMouseClicked(int offscreenX, int offscreenY) {
		...
		if (imp != null) {
			...
			RServe.callRScript("/NeuronActivityTool/src/graphic.R");
		...
		try {
			...
			while (slice <= size) {
				if (monitor.isCanceled() == false) {
					...
					RServe.callRScriptSilent("/NeuronActivityTool/src/convolve.R");
					monitor.worked(1);
					slice++;
				} else {
					break;
	...
	private void runCapturePoints() {
		...
		if (imp != null) {
			...
			RServe.callRScriptSilent("/NeuronActivityTool/src/WaveletCalc2.R");
	...
	private void markResultsInImage() {
		...
		if (imp != null) {
			...
			for (int ind = 0; ind <= (coordinatesX.length); ind++) {
				...
				imp.setRoi(new OvalRoi(((coordinatesX[ind] * parameter_3 - (parameter_3 / 2)) - CircDia),
