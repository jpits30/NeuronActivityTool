...
list.of.packages <- c("TTR", "tiff","wmtsa","changepoint","cpm","pracma","MASS","IM","readxl")
...
source(paste(directory,"NeuronActivityTool/src/criticalPoint.R",sep =""))
	...
	for (j in seq(1,length(defArray[,,1]),1)) {
  		...
  		peak_list[[j]]=0
   		...
   		if(mean(series)>MNT){
  			...
  			er1 <- tryCatch(wavCWTTree(wav_trans, n.octave.min=0.8, tolerance=0.0, type="maxima"),error=function(e) e)
...
##   This part does the ploting
	if (length(peak_list)!=0){
		...
		img = img*255/max(img)
		graphics.off()
		filePdf<-paste(directory,"NeuronActivityTool/results/result","_WS",toString(tamano),"_SNR",toString(SNR),"_SAT",toString(MNT),"_MAC",toString(minPeaks),".pdf",sep ="")
		...
		for(var in 1:length(peak_list)){
			...
			if(peak_list[[var]][[1]]!=0){
  				...
  				if(length(peak_list[[var]][[1]])>=minPeaks) {
    				...
    				##############################################################################################
#    				peak_list_temp=peak_list
#    				ventana = 30	
#    				TPD = which(def_spikes[peak_list_temp[[var]][[1]],cellNumber]==1,arr.ind=TRUE) 
#    				peak_list_temp[[var]][[1]][TPD]=NA	
#    				TP = length(TPD)
#    				
#    				for(l in 1:ventana){
#    				TPD = which(def_spikes[peak_list_temp[[var]][[1]]-l,cellNumber]==1,arr.ind=TRUE) 
#    				peak_list_temp[[var]][[1]][TPD]=NA
#    				TP = TP + length(TPD)
#    				}
#    				
#    				xtics1 = seq(1,length(signals),100)
#    				
#					filePdf2<-paste(getwd(),"/workspace/NeuronActivityTool/results/longGraph.pdf",sep ="")
#    				#fileGraph = paste(getwd(),"/workspace/NeuronActivityTool/results/longGraph.pdf")
#    				pdf(filePdf2,height=7,width=140)
#					plot(signals[1,],type="l",main=paste('Total Activity', length(peak_list[[var]][[1]]),'		','TP',TP,'		','Relaxation', ventana),xaxt="n")
#					axis(1, at=xtics1,labels=xtics1)
#					points(peak_list[[var]][[1]],peak_list[[var]][[2]],col="red",pch=18)
#					par(new=TRUE)
#					plot(def_spikes[,cellNumber],type="h",col="blue",lty="dotted",axes=FALSE)
#					dev.off()
					...
					#par(new=TRUE)
					#plot(def_spikes[,cellNumber],type="h",col="blue",lty="dotted",axes=FALSE)
    				#abline(v=2+elec_indexes*0.5/100,col="blue")
    				#par(new=TRUE)
    				#plot(def_spikes[,1],type="h",col="blue",lty="dotted")
	...
	fileTxtPun<-paste(directory,"NeuronActivityTool/results/result","WS",toString(tamano),"SNR",toString(SNR),"SAT",toString(MNT),"MAC",toString(minPeaks),".txt",sep ="")
	write.matrix(t(indiBio),fileTxtPun)
	save(Matrix_List, file=paste(directory,"NeuronActivityTool/results/Matrix_List.RData",sep =""))
    fileTxtStr<-paste(directory,"NeuronActivityTool/results/Peaks","WS",toString(tamano),"SNR",toString(SNR),"SAT",toString(MNT),"MAC",toString(minPeaks),".txt",sep ="")
